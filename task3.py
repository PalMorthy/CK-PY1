...
def to_csv_file(filename: str, headers: list, rows: list[list], delimiter=',', new_line='\n'):
    with open(filename, "w") as filename_csv:
        filename_csv.write(delimiter.join(headers) + new_line)
        [filename_csv.writelines(delimiter.join(item) + new_line) for item in rows if item]
